...
async def show_history(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for r in rows:
        ...
        text += f"- {label} | ${r['price']:.2f} | {r['date'][:19]}\n"
    await reply(update, text, reply_markup=back_keyboard())
...
async def handle_text_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == "deductbalance_amount":
        ...
        update_balance(target_id, -amount)
        clear_awaiting(context)
        await update.message.reply_text(f"✅ Deducted ${amount:.2f} from user `{target_id}`.",
                                         parse_mode=ParseMode.MARKDOWN, reply_markup=owner_menu_keyboard())
        return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_owner":
        if not is_owner(user_id):
            ...
            return
        clear_awaiting(context)
        await reply(update, "⚙️ *Owner Panel*\n\nChoose an action:", reply_markup=owner_menu_keyboard())
        return
    ...
    if data == "owner_resellers":
        ...
        text = "👥 *Resellers:*\n\n"
        for r in rows:
            text += f"- {r['username']} (`{r['user_id']}`) | Balance: ${r['balance']:.2f} | {r['role']}\n"
        await reply(update, text, reply_markup=back_keyboard("menu_owner"))
        return
